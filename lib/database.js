...
async function updateUserProfile(userId, updates) {
  try {
    ...
    const fields = Object.keys(updates)
      .filter((key) => {
        // Allow null values for profilePicture explicitly
        return (
          updates[key] !== undefined &&
          (updates[key] !== null || key === "profilePicture")
        );
      })
      .map((key) => `${key} = ?`)
      .join(", ");
    ...
    const values = [
      ...Object.keys(updates)
        .filter((key) => {
          // Include profilePicture even if null
          return updates[key] !== undefined && (updates[key] !== null || key === "profilePicture");
        })
        .map((key) => updates[key]),
      userId,
    ];
